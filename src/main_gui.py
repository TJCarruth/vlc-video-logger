...
class CarCounterGUI:
    def __init__(self, root):
        ...
        """
        These are the buttons for export, clear, undo, redo, search, delete.
        They are commented out for now to simplify the interface, as they are duplicated by the hotkeys.
        """
        ...
        # overlay instance will be created when a video/player is opened
        self.overlay = None
        self.root.bind_all('w', lambda e: self.overlay.toggle() if self.overlay else None)
        self.root.bind_all('e', lambda e: self.overlay.decrease_opacity() if self.overlay else None)
        self.root.bind_all('r', lambda e: self.overlay.increase_opacity() if self.overlay else None)
    ...
    def open_video(self, event=None):
        ...
        if path:
            ...
            try:
                ...
                # per-video config path: same base name + .overlay.json
                overlay_cfg = os.path.splitext(path)[0] + '.overlay.json'
